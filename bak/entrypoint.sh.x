...
knet_dataset_id=aratiny # In Docker (ie, CMD+ENTRYPOINT), this is aratiny by default
...
knet_dataset_dir=${2:-/kb/module/work}
...
mkdir -p /kb/module/work/tmp
tmpf=/kb/module/work/tmp

cp -Rf common/aratiny/aratiny-ws $tmpf

# We don't need the test queries used for aratiny, let's remove them from the build location
rm -Rf $tmpf/aratiny-ws/src/test/resources/knetminer-dataset/config/neo4j/*.cypher

# And then copy the dataset-specific config to the build place (in /tmp)
cp -Rf "$knet_dataset_dir/settings/ws/"* $tmpf/aratiny-ws/src/test/resources/knetminer-dataset/config
